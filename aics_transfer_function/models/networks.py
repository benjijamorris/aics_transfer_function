...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[],unet_upsampling=True):
    """Create a generator
    ...
    if netG == 'resnet_9blocks':
        ...
        net = ResnetGeneratorCR(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
    elif netG == 'resnopad':
        net = ResnetGeneratorNopad(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
    elif netG == 'resnet_6blocks':
        net = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=6)
    elif netG == 'unet_64':
        net = UnetGenerator(input_nc, output_nc, 6, ngf, norm_layer=norm_layer, use_dropout=use_dropout,unet_upsampling=unet_upsampling)
    elif netG == 'unet_128':
...
def define_NG(input_nc, output_shape, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    net = NoiseResnetGenerator(1,output_shape)
    return init_net(net, init_type, init_gain, gpu_ids)

def define_S(input_nc, ndf, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, device=None, gpu_ids=[]):
    norm_layer = get_norm_layer(norm_type=norm)
...
#     z_linspace = torch.linspace(-1,1,steps=nz,device=device)+dz*2.0/nz     # linspace(start, end, steps=100, out=None, dtype=None, layout=torch.strided, device=None, requires_grad=False)
...
class FindOffsets(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm3d, device=None):
        ...
        sequence += [nn.Conv3d(ndf * nf_mult, 3, kernel_size=kw, stride=1, padding=padw)]  # output 1 channel prediction map
...
class NoiseResnetGenerator(nn.Module):
    def __init__(self, input_nc, output_shape, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        # Nb1 (B, 16, 4, 4, 4)
        Trans1 = [nn.ConvTranspose3d(16, 16, kernel_size=3, stride=2, padding=1, output_padding=1, bias=True),
                      norm_layer(16),
                      nn.ReLU(True),
                  nn.ConvTranspose3d(16, 8, kernel_size=3, stride=2, padding=1, output_padding=1, bias=True),
                      norm_layer(8),
                      nn.ReLU(True)]
        ...
        Trans2_1 = [nn.ConvTranspose3d(8, 8, kernel_size=3, stride=2, padding=1, output_padding=1, bias=True),
                      norm_layer(8),
                      nn.ReLU(True)]
        self.Trans2_1 = nn.Sequential(*Trans2_1)
        Trans2_2 = [nn.ConvTranspose3d(8, 8, kernel_size=(1,3,3), stride=(1,2,2), padding=(0,1,1), output_padding=(0,1,1), bias=True),
                      norm_layer(8),
                      nn.ReLU(True)]
        self.Trans2_2 = nn.Sequential(*Trans2_2)
        # self.Nb3 = NoiseBlock(48,8,32,(32,64,64),4)
        self.Nb3 = NoiseBlock(48,8,32,(sz,sy//2,sx//2),4)
        Trans3 = [nn.ConvTranspose3d(4, 4, kernel_size=(1,3,3), stride=(1,2,2), padding=(0,1,1), output_padding=(0,1,1), bias=True),
                      norm_layer(4),
                      nn.ReLU(True)]
...
class NoiseBlock(nn.Module):
    def __init__(self, input_noise_nc, input_nc, ngf, output_shape, output_nc, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        Res_m1 = [nn.Conv3d(output_nc+1, ngf, kernel_size=3, stride=1, padding=1, bias=True),
                      norm_layer(ngf),
                      nn.ReLU(True),
                      ResnetBlock(ngf, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=True)]
        self.Res_m1 = nn.Sequential(*Res_m1)

        if input_nc > 0:
            Res_m2 = [nn.Conv3d(input_nc, ngf, kernel_size=3, stride=1, padding=1, bias=True),
                        norm_layer(ngf),
                        nn.ReLU(True),
                        ResnetBlock(ngf, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=True)]
        ...
        Res_m3 = [ResnetBlock(ngf, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=True),
    ...
    def forward(self, input, z, mask):
        z2 = self.Linear_m(z)
        z2 = z2.view((-1, self.output_nc, self.output_shape[0], self.output_shape[1], self.output_shape[2]))
        mask2 = F.interpolate(mask, size=self.output_shape, mode='trilinear', align_corners=True)
...
class MLP(nn.Module):
    def __init__(self, input_dim, output_dim, mid_dim, n_blk, norm='none', activ='relu'):
        ...
        self.model += [LinearBlock(mid_dim, output_dim, norm='none', activation='none')] # no output activations
...
class ResnetGeneratorCR(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            model += [nn.Conv3d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=(2,2,2), padding=1, bias=use_bias),
                      norm_layer(ngf * mult * 2),
                      nn.ReLU(True)]
...
class ResnetGeneratorNopad(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6):
        ...
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            model += [nn.Conv3d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=(1,2,2), padding=0, bias=use_bias),
                      norm_layer(ngf * mult * 2),
                      nn.ReLU(True)]
        ...
        for i in range(n_blocks):       # add ResNet blocks
            ...
            model += [ResnetBlock(ngf * mult, padding_type='zero', norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            model += [nn.Conv3d(ngf * mult, int(ngf * mult / 2),
                                         ...
                                         padding=0,
                                         bias=use_bias),
                      Upsample3D(scale_factor=tuple([1.0,2.0,2.0]), mode='trilinear',align_corners=True),
                      norm_layer(int(ngf * mult / 2)),
                      nn.ReLU(True)]
...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.padding_type == 'none':
            ...
            out = x[:,:,2:-2,2:-2,2:-2] + self.conv_block(x)  # add skip connections
        else:
...
class UnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False,unet_upsampling=True):
        ...
        self.model = UnetSkipConnectionBlock(output_nc, ngf, input_nc=input_nc, submodule=unet_block, outermost=True, norm_layer=norm_layer,unet_upsampling=unet_upsampling)  # add the outermost layer
...
class UnetGeneratorNopad(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False,unet_upsampling=True):
        ...
        # construct unet structure
        unet_block = UnetSkipConnectionBlockNopad(ngf * 8, ngf * 8, input_nc=None, submodule=None, norm_layer=norm_layer, innermost=True)  # add the innermost layer
        for i in range(num_downs - 5):          # add intermediate layers with ngf * 8 filters
            unet_block = UnetSkipConnectionBlockNopad(ngf * 8, ngf * 8, input_nc=None, submodule=unet_block, norm_layer=norm_layer, use_dropout=use_dropout,downsampling=False)
        # gradually reduce the number of filters from ngf * 8 to ngf
        unet_block = UnetSkipConnectionBlockNopad(ngf * 4, ngf * 8, input_nc=None, submodule=unet_block, norm_layer=norm_layer)
        unet_block = UnetSkipConnectionBlockNopad(ngf * 2, ngf * 4, input_nc=None, submodule=unet_block, norm_layer=norm_layer)
        unet_block = UnetSkipConnectionBlockNopad(ngf, ngf * 2, input_nc=None, submodule=unet_block, norm_layer=norm_layer)
        self.model = UnetSkipConnectionBlockNopad(output_nc, ngf, input_nc=input_nc, submodule=unet_block, outermost=True, norm_layer=norm_layer,unet_upsampling=unet_upsampling)  # add the outermost layer
...
class UnetSkipConnectionBlockNopad(nn.Module):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, norm_layer=nn.BatchNorm3d, use_dropout=False, unet_upsampling=True, downsampling=True):
        """Construct a Unet submodule with skip connections.
        ...
        if outermost:
            ...
            upconv = nn.ConvTranspose3d(inner_nc, outer_nc,
                                        ...
                                        padding=0, bias=use_bias) #origin: 4,2,1
            down = [downrelu, downconv]
            up = [uprelu, upconv, upnorm]
            model = down + up
        elif downsampling:
